...
from ccusage_monitor.protocols import CcusageBlock, CcusageData, DisplayValues


class TypedCLIArgs(argparse.Namespace):
...
def parse_args() -> TypedCLIArgs:
    ...
    return parser.parse_args(namespace=TypedCLIArgs())
...
class MonitorState:
    ...
    def __init__(self) -> None:
        self.last_data_hash: Optional[Tuple[int, Optional[str], int]] = None
        self.last_active_block: Optional[CcusageBlock] = None
        self.last_burn_rate: float = 0
        self.last_reset_time: Optional[datetime] = None
        self.token_limit: int = 0
        self.timezone: Optional[str] = None

    def data_changed(self, data: CcusageData) -> bool:
...
def format_display_values(
    ...
    if cached is not None:
        return cast(DisplayValues, cached)
...
def main():
    ...
    try:
        ...
        while True:
            ...
            # Predictions
            display.writeln(f"🏁 {white}Predicted End:{reset} {values['predicted_end_str']}")
            display.writeln(f"🔄 {white}Token Reset:{reset}   {values['reset_time_str']}")
            display.writeln()
